...
export default function DashboardLayout({ children }: DashboardLayoutProps) {
  ...
  return (
    <div className="min-h-screen w-full bg-transparent text-white relative">
      <AuroraBackground />
      <div className="relative z-10 max-w-8xl mx-auto grid grid-cols-12 gap-4 pt-6 pb-12 px-4">
        {/* Sidebar */}
        <div className="col-span-12 md:col-span-2 lg:col-span-1 relative z-50">
          <Sidebar />
        </div>
  ...
  function Sidebar() {
    return (
      <div className="flex flex-col space-y-3 pt-16 px-2 bg-transparent">
        {menuItems.map((item) => (
          <button
            key={item.name}
            onClick={() => handleMenuClick(item.path)}
            className={cn(
              'group flex items-center space-x-2 p-2 rounded-lg transition-all duration-300 w-full text-left transform hover:scale-105',
          ...
          >
            <item.icon className="w-4 h-4 transition-all duration-300 group-hover:text-white" />
